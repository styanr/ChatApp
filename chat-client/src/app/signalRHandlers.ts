...
import { chatRoomApiSlice } from "../features/chatrooms/chatRoomApiSlice"
...
export const setupSignalRListeners = (connection: signalR.HubConnection) => {
  ...
  connection.on("ReceiveChatRoom", (chatRoom: ChatRoomDetails) => {
    console.log("ReceiveChatRoom", chatRoom)

    store.dispatch(
      chatRoomApiSlice.util.updateQueryData(
        "getChatRoomById",
        chatRoom.id,
        () => chatRoom,
      ),
